...
export default {
  ...
  getters: {
    // 统计购物车中商品的总数量
    total(state) {
      return state.cart.reduce((total, item) => total = total + item.goods_count, 0)
    },
    // 勾选的商品的总数量
    checkedCount(state) {
      return state.cart.filter(item => item.goods_state).reduce((total, item) => total = total + item.goods_count, 0)
    },
